...
bool FindCycleHelper(size_t i, gsl::span<const InlinedVector<size_t>> adjacency_map, gsl::span<char> visited, gsl::span<char> st,
...
bool TensorrtExecutionProvider::DetectTensorRTGraphCycles(SubGraphCollection_t& supported_nodes_vector, const GraphViewer& graph, const HashValue& model_hash, bool remove_cycles) const {
  ...
  while (trt_cycle) {
    ...
    InlinedVector<char> visited(graph_size);
    InlinedVector<char> st(graph_size);
